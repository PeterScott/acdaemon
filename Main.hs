{-# LANGUAGE ForeignFunctionInterface, BangPatterns, OverloadedStrings #-}
module Main where
...
insertStr :: ByteString -> IO ()
insertStr bs = insertPair bs 0

autocompleteStr :: ByteString -> IO [ByteString]
autocompleteStr bs = do lst <- autocomplete bs
                        return $ map fst lst
...
main = do
  ...
  forM_ (L.lines wordsBS) $ \word ->
      insertStr $ B.concat (L.toChunks word)
